...
private lemma lca_same_length_prefix (as bs : BcChain) (len : ℕ) (ha : len = as.length) (hb : len = bs.length)
  : lca_same_length as bs len ha hb ≼ as ∧ lca_same_length as bs len ha hb ≼ bs := by
  induction len generalizing as bs
  . unfold lca_same_length
    quiet simp_all only [bc_prefix, bc_rep_suffix, ↓reduceDIte, List.isSuffixOf_nil_left, List.nil_suffix,
                         decide_true, true_or, Bool.true_or, and_self]
  . rename_i n induct
    unfold lca_same_length; simp_all only [bc_prefix, bc_rep_suffix]
    split <;> try quiet simp only [List.isSuffixOf_nil_left, List.nil_suffix,
                                   decide_true, true_or, Bool.true_or, and_self]
    rename_i as_ne
    split <;> try trivial
    ...
    simp at heq
    split
    . quiet simp_all only [List.isSuffixOf_iff_suffix, List.suffix_cons,
                           decide_true, true_or, Bool.true_or, Bool.or_eq_true, and_self,
    ...
    . rename_i a_neq_b
      ...
      have br_suff_bbr  : br <:+ b::br := by quiet simp_all [List.length_cons, decide_eq_true_eq]

      let ind := lca_same_length ar br ar.length rfl (by simp_all only [List.length_cons])
      suffices ind <:+ a::ar ∧ ind <:+ b::br by
        quiet simp_all only [bc_rep_suffix, List.isSuffixOf_iff_suffix, List.length_cons, add_tsub_cancel_right,
                             decide_true, true_or, Bool.true_or, Bool.or_eq_true, and_self, ind]
      let ind_preceq := induct ar br (by simp only [n_eq_arl]) (by simp only [brl_eq_arl])
      simp [brl_eq_arl] at ind_preceq
      apply And.intro
      . cases ind_preceq.left with  | inl lca_suff_ar => exact List.IsSuffix.trans lca_suff_ar ar_suff_aar
                                    | inr lca_same_ar => simp_all [NC ind ar lca_same_ar]
      . cases ind_preceq.right with | inl lca_suff_br => exact List.IsSuffix.trans lca_suff_br br_suff_bbr
                                    | inr lca_same_br => simp_all [NC ind br lca_same_br]
...
private lemma lca_left_no_longer (a b : BcChain) (a_no_longer : a.length ≤ b.length)
  ...
  . simp [bc_prefix, bc_rep_suffix] at ⊢ lca_slr
    left
    cases lca_slr.right with | inl lca_suff_btr => exact List.IsSuffix.trans lca_suff_btr btr_suff_b
                             | inr lca_same_btr => quiet simp_all only [List.isSuffixOf_iff_suffix,
                                                                        NC lca btr lca_same_btr]
